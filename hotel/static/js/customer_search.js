...
console.log('Loading customer_search.js');

// تعريف الدوال العالمية بطريقة أخرى
window.quickAddDocument = function(customerId) {
    console.log('quickAddDocument called with ID:', customerId);
...
window.deleteCustomer = function(customerId, customerName) {
    console.log('deleteCustomer called with ID:', customerId, 'Name:', customerName);
    if (confirm(`هل أنت متأكد من حذف العميل ${customerName}؟`)) {
        fetch(`/customers-new/delete/${customerId}`, {
            method: 'POST',
            headers: {
        ...
        .catch(error => {
            ...
            showAlert('حدث خطأ في حذف العميل', 'error');
        });
    }
};
...
// دالة عرض التنبيهات معرفة في ملف index_new.html

// تم نقل تعريف دالة deleteCustomer إلى الأعلى
